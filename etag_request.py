...
class ScrapAPI():
    ...
    def request(self, method: str = "GET", endpoint: str = "", headers: dict = None, payload: str = None, etag: str = None, use_cache: bool = False):
        ...
        with self.session.request(method=method, url=f"{self.base_api}{endpoint}", headers=headers, data=payload) as data:
    ...
    def headers(self, endpoint: str, headers: dict = {}, payload: str = ""):
        with self.session.head(f"{self.base_api}{endpoint}", headers=headers, data=payload) as data:
            return data
    ...
    async def arequest(self, method: str = "GET", endpoint: str = "", session: aiohttp.ClientSession = None, headers: dict = None, payload: str = None, etag: str = None, use_cache: bool = False):
        if method.upper() not in ["GET", "HEAD", "OPTIONS", "TRACE", "PUT", "DELETE", "POST", "PATCH", "CONNECT"]:
            endpoint: str = method
            method: str = "GET"
        ...
        if session is None:
            async with aiohttp.ClientSession() as session:
                async with session.request(method=method, url=f"{self.base_api}{endpoint}", headers=headers, data=payload) as data:
                    self.etag[endpoint]: str = data.headers.get("etag", self.etag.get(endpoint))
                    if data.status == 200:
                        try:
    ...
    async def aheaders(self, endpoint: str, headers: dict = {}, payload: str = ""):
        async with aiohttp.ClientSession() as session:
            async with session.head(f"{self.base_api}{endpoint}", headers=headers, data=payload) as data:
                return data
    ...
    def on_api_update(self, callback: callable = None, endpoints: list = None, headers: dict = {}, payload: str = ""):
        ...
        def add_debug(func):
            ...
            for endpoint in endpoints:
                self._to_call[endpoint]: dict = {"callback": func, "headers": headers, "payload": payload}
